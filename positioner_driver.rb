...
=end
    ...
    @pub_thread = Thread.new do
      while true do
        @streaming_if.send_with_type "rel_bearing", @rotor.rel_bearing
        sleep 0.5
      end
    end

    @rep_thread = Thread.new do
      while (msg = @rest_if.receive) do
        if msg == 'frontend-streaming-port'
          rep = @streaming_if.port.to_s
        end
        @rest_if.send rep
      end
    end
